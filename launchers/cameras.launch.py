...
def getCamInfoYaml(cameraName: str):
    ...
    fileNamePath = os.path.join(getRealPackageDirPath(), 'configs', 'sunplus_camera_info.yaml')
...
def getLaunchArgs():
    return [
        DeclareLaunchArgument(
            'namespace',
            default_value='',
            description="ROS namespace name for cameras"
        )
    ]
    
def getLaunchActions(context):

    ros_namespace = LaunchConfiguration('namespace').perform(context)
    ...
    cameraNodes = [
        Node(
            ...
            # ros_arguments=[{
            #     'params-file': camera.getYamlParametersPath()
            # }],
            # parameters=[{
            #     'image_size': [640,480],
            #     'camera_frame_id': camera.getCameraFrameName(),
            # }]
        )
